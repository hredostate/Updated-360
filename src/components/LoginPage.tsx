...
export default function LoginPage({ onNavigate }: LoginPageProps) {
        ...
        const { data, error } = await (supabase.auth as any).signUp({
            ...
            options: {
                data: {
                    ...
                    user_type: 'staff', // Hardcoded for staff portal
                },
            }
        });
        if (error) throw error;
  ...
  const getTitle = () => {
    switch(authView) {
        case 'login': return 'Secure Staff Sign In';
        case 'signup': return 'Secure Staff Sign Up';
    ...
    <div className="h-full w-full overflow-hidden bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950">
      ...
      <main className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 items-stretch gap-10 px-6 pb-16 pt-2 md:grid-cols-2">
        ...
        <section className="order-2 md:order-1 flex min-h-[28rem] flex-col justify-center rounded-3xl border border-indigo-200/60 bg-indigo-50/60 p-8 backdrop-blur-xl shadow-xl dark:border-indigo-800/60 dark:bg-indigo-900/40">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">The Command Center for Modern Schools</h1>
          <p className="mt-3 text-slate-600 dark:text-slate-300">Guardian 360 is an AI-powered dashboard for school administrators to manage reports, tasks, students, and institutional data.</p>
          ...
          <div className="mt-6 space-y-4 text-slate-700 dark:text-slate-200">
            ...
            <ul className="list-disc pl-5 text-sm space-y-1">
              <li>AI-powered report analysis & summarization</li>
              <li>Automated task delegation and reminders</li>
              <li>Real-time student behavior tracking</li>
              <li>Dynamic policy knowledge base</li>
            </ul>

            <h2 className="text-base font-semibold text-slate-900 dark:text-white">🏫 For Visionary School Leaders</h2>
            <ul className="list-disc pl-5 text-sm space-y-1">
              <li>Proactive intelligence to foster a safe environment</li>
              <li>Actionable insights for data-driven decisions</li>
              <li>Streamlined operations for a more efficient school</li>
            </ul>
          </div>
        </section>
