...
interface SurveyManagerProps {
    ...
    onSaveSurvey: (surveyData: Omit<SurveyWithQuestions, 'id' | 'created_at' | 'school_id' | 'created_by'> & { id?: number }) => Promise<void>;
    onDeleteSurvey: (surveyId: number) => Promise<void>;
...
const SurveyBuilderModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSave: (surveyData: Omit<SurveyWithQuestions, 'id' | 'created_at' | 'school_id' | 'created_by'> & { id?: number }) => Promise<void>;
    existingSurvey: SurveyWithQuestions | null;
    addToast: (message: string, type?: 'success' | 'error' | 'info') => void;
    allClasses: BaseDataObject[];
    allArms: BaseDataObject[];
    allRoles: RoleDetails[];
}> = ({ isOpen, onClose, onSave, existingSurvey, addToast, allClasses, allArms, allRoles }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setAudience(existingSurvey?.audience || []);
            if (existingSurvey && existingSurvey.questions) {
                // Deep copy and sort to avoid mutation issues and ensure order
                const sortedQuestions = [...existingSurvey.questions].sort((a, b) => a.position - b.position);
    ...
    const updateOption = (qIndex: number, oIndex: number, field: keyof MultipleChoiceOption, value: any) => {
        const newQuestions = [...questions];
        const options = newQuestions[qIndex].options as MultipleChoiceOption[];
        if(options) {
            (options[oIndex] as any)[field] = value;
            setQuestions(newQuestions);
        }
    }
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in">
            <div className="rounded-2xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur-xl shadow-2xl dark:border-slate-800/60 dark:bg-slate-900/80 w-full max-w-4xl m-4 flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold">{existingSurvey ? 'Edit Survey' : 'Create Survey'}</h2>
                    <button onClick={onClose} className="text-slate-500 hover:text-slate-700">&times;</button>
                </div>
                
                <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                    <div className="space-y-2">
                        <input type="text" placeholder="Survey Title" value={title} onChange={e => setTitle(e.target.value)} className="w-full p-2 border rounded-md" />
                    ...
                    <div className="space-y-4">
                                        ...
                                        {(q.options as MultipleChoiceOption[])?.map((opt, oIndex) => (
                                            <div key={oIndex} className="flex items-center gap-2">
                                                ...
                                                <input 
                                                    type="text" 
                                                    value={opt.text} 
                                                    onChange={e => updateOption(i, oIndex, 'text', e.target.value)} 
                                                    placeholder={`Option ${oIndex + 1}`} 
                                                    className="flex-grow p-1 border rounded bg-white dark:bg-slate-700"
                                                />
                                                <input 
                                                    type="number" 
                                                    value={opt.quota || ''} 
                                                    onChange={e => updateOption(i, oIndex, 'quota', e.target.value ? Number(e.target.value) : null)} 
                                                    placeholder="Limit (Optional)" 
                                                    title="Maximum number of times this option can be selected"
                                                    className="w-24 p-1 border rounded bg-white dark:bg-slate-700 text-sm"
                                                />
                        ...
                        <div className="flex gap-2 justify-center">
                            <button onClick={() => addQuestion('multiple_choice' as SurveyQuestionType)} className="px-3 py-1 bg-slate-200 rounded text-sm">Multiple Choice</button>
                            <button onClick={() => addQuestion('short_answer' as SurveyQuestionType)} className="px-3 py-1 bg-slate-200 rounded text-sm">Short Answer</button>
                            <button onClick={() => addQuestion('true_false' as SurveyQuestionType)} className="px-3 py-1 bg-slate-200 rounded text-sm">True/False</button>
                            <button onClick={() => addQuestion('ranking' as SurveyQuestionType)} className="px-3 py-1 bg-slate-200 rounded text-sm">Rating</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-4 border-t mt-4 flex justify-end gap-2">
                    ...
                    <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 bg-blue-600 text-white rounded-md flex items-center gap-2">
                        {isSaving && <Spinner size="sm"/>} Save Survey
                    </button>
                </div>
            </div>
        </div>
...
const SurveyManager: React.FC<SurveyManagerProps> = ({ surveys, onSaveSurvey, onDeleteSurvey, addToast, allClasses, allArms, allRoles }) => {
    ...
    const handleEdit = (survey: SurveyWithQuestions) => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Survey Manager</h1>
                    <p className="text-slate-600 dark:text-slate-300 mt-1">Create and manage surveys, polls, and assessments.</p>
                </div>
                <button onClick={() => { setEditingSurvey(null); setIsModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">
                    <PlusCircleIcon className="w-5 h-5"/> Create Survey
                </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {surveys.map(survey => (
                    <div key={survey.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-white">{survey.title}</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 h-10">{survey.description}</p>
                        <p className="text-xs text-slate-400 mt-2">{survey.questions.length} questions</p>
                        
                        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                            <button onClick={() => setViewingResultsSurvey(survey)} className="text-sm font-medium text-purple-600 hover:underline">Results</button>
                            <button onClick={() => handleEdit(survey)} className="text-sm font-medium text-blue-600 hover:underline">Edit</button>
                            <button onClick={() => handleDelete(survey.id)} className="text-sm font-medium text-red-600 hover:underline">Delete</button>
                        </div>
                    </div>
                ))}
                {surveys.length === 0 && (
                    <div className="col-span-full text-center py-16 border border-dashed rounded-xl">
                        <p className="text-slate-500">No surveys created yet.</p>
                    </div>
                )}
            </div>
