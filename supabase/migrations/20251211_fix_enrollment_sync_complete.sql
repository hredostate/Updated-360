-- ============================================
-- Complete Enrollment Sync Fix
-- Fixes all enrollment sync errors by properly managing function versions
-- ============================================

-- STEP 1: Drop all existing versions of sync functions
-- This ensures no conflicts between old and new function signatures

-- Drop old 3-parameter versions (from 20251210_add_student_enrollment_sync.sql)
DROP FUNCTION IF EXISTS sync_student_enrollment(INTEGER, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS sync_all_students_for_term(INTEGER, INTEGER);
DROP FUNCTION IF EXISTS admin_sync_student_enrollments(INTEGER, INTEGER);

-- Drop new 4-parameter versions (from 20251211_improve_enrollment_sync.sql)
-- Note: We need to drop these to recreate them cleanly
DROP FUNCTION IF EXISTS sync_student_enrollment(INTEGER, INTEGER, INTEGER, BOOLEAN);
DROP FUNCTION IF EXISTS sync_all_students_for_term(INTEGER, INTEGER, BOOLEAN);
DROP FUNCTION IF EXISTS admin_sync_student_enrollments(INTEGER, INTEGER, BOOLEAN);

-- Drop diagnostics function (will be recreated)
DROP FUNCTION IF EXISTS get_enrollment_sync_diagnostics(INTEGER, INTEGER);

-- Drop trigger functions to recreate them
...
-- This was added in 20251211_improve_enrollment_sync.sql but we ensure it's there
...
-- STEP 3: Recreate all functions with correct signatures

-- Function: sync_student_enrollment (4 parameters)
-- Syncs a single student's enrollment with manual enrollment protection
...
-- Function: sync_all_students_for_term (3 parameters)
-- Bulk synchronizes all students for a term
...
-- Function: admin_sync_student_enrollments (3 parameters)
-- Admin function with detailed statistics
...
-- STEP 4: Recreate trigger functions with correct function calls
...
-- STEP 6: Grant execute permissions on all functions to authenticated role
-- This is critical - must grant for the correct function signatures
...
-- STEP 7: Create index for optimal sync performance
...
COMMENT ON FUNCTION get_enrollment_sync_diagnostics IS 
    'Diagnostic function to identify students with enrollment sync issues';
